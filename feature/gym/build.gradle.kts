...
kotlin {
    ...
    android {
        namespace = "pl.lejdi.plannerkmp.feature.gym"
    ...
    sourceSets {
        commonMain.dependencies {
            ...
            // `databaseModule`, `networkModule()`, `tasksModule`, `groceryModule`,
            // `routinesModule` and `gymModule` are all typed `org.koin.core.module.Module`, and
            // :shared names every one of them. As
            ...
            // api, not implementation: DayOfWeek and LocalDate appear in GymExercise, in
            // GymExerciseDraft.create, in GymDay and in GymDatasource.updateCompletedSets — this
            // module's own public signatures, and the nav key's too.
...
sqldelight {
    databases {
        create("GymDatabase") {
            ...
            packageName.set("pl.lejdi.plannerkmp.feature.gym.data")
        }
    }
}
...
compose.resources {
    publicResClass = true
    packageOfResClass = "pl.lejdi.plannerkmp.feature.gym.resources"
