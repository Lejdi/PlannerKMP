...
class SqlDelightGroceryDatasource(
    ...
    override suspend fun getAllItems(): AppResult<List<GroceryItem>> = safeQuery(dispatchers) {
        queries.selectAll().executeAsList().map { it.toDomain() }
    }

    override suspend fun addItem(item: GroceryItem): AppResult<Unit> = safeQuery(dispatchers) {
    ...
    override suspend fun editItem(item: GroceryItem): AppResult<Unit> = safeQuery(dispatchers) {
    ...
    override suspend fun deleteItem(id: Long): AppResult<Unit> = safeQuery(dispatchers) {
